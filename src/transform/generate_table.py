...
def process_rank_data():
    ...
    dic = {'USNews RANK':'rank 2012', 'Unnamed: 1':'rank 2013', 'USNews SCORE':'rank 2014',
           'Unnamed: 3':'rank 2015', 'Unnamed: 4':'Law School', 'Unnamed: 5':'score 2012',
    ...
    for item in lists:
        df_8799 = df_8799.rename(columns={item:'rank '+str(int(df_8799.loc[0,item]))})
    ...
    for item in range(9):
        df_0008 = df_0008.rename(columns={'April.'+str(item):'rank '+str(df_0008.loc[0,'April.'+str(item)])})
    ...
    for item in range(2009,2013):
        ...
        df_0911['rank '+str(item)] = df_0911['score '+str(item)].apply(float).rank(ascending=0)  
    ...
    df_18 = df_18.rename(columns={0:'rank 2018',1:'Law School', 2:'Tuition',3:'FT Enrollment'})
    df_18_v1 = df_18[~df_18['rank 2018'].isnull()].reset_index()
    df_18_v2 = df_18[df_18['rank 2018'].isnull()].reset_index().rename(columns={'Law School':'Location'}).drop(['Tuition','FT Enrollment'],axis=1)
    ...
    df_ranks = df_ranks.drop(['score 2012_y','rank 2012_y'],axis=1).rename(columns={'score 2012_x':'score 2012','rank 2012_x':'rank 2012'})\
    ...
    df_pig_rank_18 = df_pig_rank_18[df_pig_rank_18['Law School'].notnull() & df_pig_rank_18['name'].notnull()].rename(columns={'rank 2018_x':'rank 2018'}).drop(['Law School','rank 2018_y'],axis=1)
    ...
    df_pig = df_pig.drop(['rank 2018','Tuition','FT Enrollment','Location'],axis=1)
    df_pig_rank_18_sub = df_pig_rank_18[['name','match','rank 2018','Tuition','FT Enrollment','Location']]
...
def clean_app_rank(df_app):
    ...
    df_app_new = df_app.drop(['Unnamed: 0', 'Unnamed: 0_x', 'Unnamed: 0.1', 'index_x','Unnamed: 0_x.1',
                              'Unnamed: 0_y','Last Updated_x','Unnamed: 0_y.1', 'index_y'],axis=1)
    ...
    for yr in (range(1990,2016)+[1987,2018]):
        df_app_new['rank '+str(yr)] = df_app_new['rank '+str(yr)].fillna('').apply(str).apply(keep_ascii)

    df_app_new.loc[df_app_new['rank 2012']=='24..5','rank 2012'] = '24.5'
    df_app_new['rank 2018'] = df_app_new['rank 2018'].str.replace('#','').str.replace('Tie','')\
                              .str.replace('RNP','').str.replace('Unranked','')
                                  
    for yr in (range(1990,2016)+[1987,2018]):
        df_app_new.loc[df_app_new['rank '+str(yr)].str.contains('NR'),'rank '+str(yr)] = '-9999.0'
        df_app_new.loc[df_app_new['rank '+str(yr)]=='','rank '+str(yr)] = '-9999.0'
    
    for yr in ([1987,2018]+range(1990,2016)):
        df_app_new['rank '+str(yr)] = df_app_new['rank '+str(yr)].astype(float)
        df_app_new.loc[df_app_new['rank '+str(yr)]==-9999,'rank '+str(yr)] = np.nan
...
def clean_app_date(df_app):
    ...
    for yr in range(2003, 2017):
        ...
        print 'Year=',yr,df_yr[yr-2003][['Sent','Sent_delta','GPA','LSAT']].describe()
...
if __name__ == '__main__':
    ...
    df_app_new = pd.read_csv('../../data/edit/app_matched.csv')
    ...
    for yr in range(2003,2017):
         ...
         print 'all',df_yr[yr-2003]['User Name'].nunique(), yr
    
    dic = {}
    for action in ['Sent','Received','Complete','Decision']:
        dic[action]=df_yr[yr-2003][~df_yr[yr-2003][action].isnull()] #.str.contains('/')
        print , 'yr=',yr[yr-2003],'#sent=',dic['Sent']['User Name'].nunique(),'#decision=' dic['Decision']['User Name'].nunique(), \
              '#received=',dic['Received']['User Name'].nunique(),'#complete=',dic['Complete']['User Name'].nunique()
    '''
    '''
    result = smf.ols('Status_Final~Sent_delta+LSAT+GPA', data=df_yr).fit()
